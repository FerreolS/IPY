...
func current_include_dir(nil)
/* DOCUMENT current_include_dir();

     Get the full path of the directory of the result returned by
     `current_include`.  The result is guaranteed to be terminated by a "/".

     SEE ALSO: current_include */
{
  path = current_include();
  return ((path && (i = strfind("/", path, back=1)(2)) > 0) ?
          strpart(path, 1:i) : cd("."));
}

/* Yeti and LINOP are required. */
if (is_func(h_new) != 2) include, "yeti.i", 1;
if (! is_func(linop_new)) include, current_include_dir() + "linop.i", 1;
...
local rgl_HessSchat;
/* DOCUMENT this = rgl_new("HessSchat");
     This regularization has the following hyper-parameters:
...
func _rgl_HessSchat_state2(self, x)
{
...
func _rgl_HessSchat_get_gradient(self, x)
{
  if (self.state < 2) _rgl_HessSchat_state2, self, x;
  return self.grd;
}

#if 0
func _rgl_HessSchat_apply_hessian(self, x, s) {}
func _rgl_HessSchat_get_hessian(self, x) {}
func _rgl_HessSchat_get_diagonal_of_hessian(self, x) {}
#endif
